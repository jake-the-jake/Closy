...
The package is valid without ZeroOne, AI reconstruction, source-image textures or personalised avatars. It now includes synthetic metadata-only capture records, deterministic capture quality scoring, analytic visual observations, an empty editable correction record, deterministic T-shirt parameter fitting, texture identity/PBR evidence derived from authored materials, a rejected null visual-geometry proposal boundary, and a deterministic CPU reference cloth settle; self-collision, real source texture projection, generated texture atlases, clean provider geometry, and production-grade material calibration remain unavailable until real stages produce them.
