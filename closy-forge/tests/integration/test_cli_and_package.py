...
def test_report_json_is_machine_readable(tmp_path, capsys) -> None:  # type: ignore[no-untyped-def]
    ...
    assert payload["geometryBindingValidation"]["maxCleanupToSettledOffsetMeters"] == 0.781177462
    ...
    assert payload["geometryRepairRetopologyPlan"]["deformationFailedVertexCount"] == 191
    assert (
