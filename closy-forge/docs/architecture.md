...
fixed avatar contract
  ...
  -> deterministic T-shirt parameter fitting
...
The current T-shirt stores synthetic metadata-only capture evidence, analytic mask/landmark observations, an empty editable correction record, a bounded deterministic parameter fit, analytic rest assembly and a deterministic CPU settled state against the fixed collision avatar. The solver exercises gravity, stretch, bend, seam and body-collision constraints, but it is still a reference backend: self-collision, real user images, body scans, learned segmentation, AI inference, ZeroOne processing and personalised body data are not present.

## Future R&D Direction

The next dependency-ready work is texture identity scaffolding or richer pose/motion validation, while preserving the same package boundary. Later work can add reconstruction services, SMPL/SMPL-X research, AI garment proposal adapters, ZeroOne derivatives, and mobile consumption of these packages without changing the package boundary.
