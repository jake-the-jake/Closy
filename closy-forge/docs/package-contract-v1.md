...
Capabilities are booleans, not quality scores. Current true states are immutable synthetic capture records, deterministic capture quality scoring, analytic visual observations, editable correction records, synthetic target-garment masks, synthetic T-shirt landmarks, deterministic T-shirt parameter fitting, texture identity evidence, PBR material observations, raw geometry proposal records, raw geometry topology diagnostics, geometry cleanup recommendations, local cleanup execution, provider provenance, provider registry availability, provider contract validation, provider bake-off reporting, manual import adapter declaration, configured manual raw geometry asset, local open-model adapter declaration, geometry proposal quality scoring, pattern, simulation-ready topology, authored material preset, conventional GLB, sim-to-render binding, validated reconstruction, deterministic reference cloth settle, D0 reference self-collision evidence, production binding contract availability and scoped D0 production-binding C3 profile evidence. Source-image texture, external geometry providers, available clean geometry proposal, local open-model execution, personalised avatar, skeleton, production GPU self-collision, global Phase 6 completion and ZeroOne states are false. The rejected `proposals/clean_geometry_proposal.json` is evidence that the gate was evaluated, not a claim that a clean proposal exists; the cleanup plan is evidence of required repair work, the cleanup result is evidence of partial preview-only local cleanup, the provider bake-off is D0 contract/manual-fixture evidence only, and the C3 report is scoped to the fixed-avatar T-shirt profile only.
