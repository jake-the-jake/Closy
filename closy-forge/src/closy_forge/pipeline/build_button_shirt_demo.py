...
from closy_forge.garments.button_shirt.appearance import (
    ButtonShirtAppearanceBundle,
    build_button_shirt_appearance_bundle,
)
...
from closy_forge.garments.button_shirt.fitting import fit_button_shirt
from closy_forge.garments.button_shirt.motion import build_button_shirt_motion_suite
from closy_forge.garments.button_shirt.parameters import ButtonShirtParameters
from closy_forge.garments.button_shirt.pattern_generator import (
    GARMENT_CLASS,
    GARMENT_ID,
...
PACKAGE_VERSION = "closy.button_shirt.package.d0.v1"
SELECTED_PRESET_ID = "material.lightweight_woven_d0_v1"
CONTRACT_WRITE_SPEC = ContractWriteSpec(
    package_version=PACKAGE_VERSION,
    simulation_node_name="closy_button_shirt_simulation_v1",
    dense_render_node_name="closy_button_shirt_dense_render_v1",
    independent_fallback_node_name="closy_button_shirt_independent_simulation_fallback_v1",
    fit_report_path="fitting/button_shirt_fit.json",
    quality_report_path="reports/button_shirt_quality.json",
    normalize_signed_zero=True,
    selected_material_preset_id=SELECTED_PRESET_ID,
)
SUMMARY_SPEC = SummarySpec(
    completion_key="buttonShirtD0Complete",
    completion_label="Button-shirt D0",
)


@dataclass(frozen=True)
class ButtonShirtBuildResult:
    package_dir: Path
    manifest: dict[str, Any]
    validation: dict[str, Any]


def build_demo_button_shirt_package(
    output: Path,
    *,
    params: ButtonShirtParameters | None = None,
    seed: int = DEFAULT_SEED,
    force: bool = False,
) -> ButtonShirtBuildResult:
    prior = params or ButtonShirtParameters()
    prior.validate()
    ...
    try:
        ...
        if validation["status"] != "passed":
            ...
            raise RuntimeError(f"button-shirt package validation failed before publish: {details}")
        write_summary_files(staging, context, validation, SUMMARY_SPEC)
        publish_staging(staging, output, force=force)
        return ButtonShirtBuildResult(output, context["manifest"], validation)
    except Exception:
...
def _write_package_contents(
    package_dir: Path, prior: ButtonShirtParameters, seed: int
) -> dict[str, Any]:
    fitted, fit_report = fit_button_shirt(prior)
    pattern = build_button_shirt_pattern(fitted)
    ...
    material_selection = select_material_preset(
        material_selection_input(
            ...
            observations={
                ...
                "surfaceClass": "plain_woven",
            },
        ),
        material_registry,
    )
    if material_selection["selection"]["selectedPresetId"] != SELECTED_PRESET_ID:
        raise RuntimeError("button-shirt material selector did not choose lightweight woven")
    material_physics = solver_material_payload(material_selection["selectedDescriptor"])
    motion_report, motion_states, simulation_mesh = build_button_shirt_motion_suite(
        rest_mesh=rest_mesh,
    ...
    appearance = build_button_shirt_appearance_bundle(
...
def _manifest(
    *,
    ...
    return {
        ...
        "displayName": "Deterministic Button Shirt D0 Fixture",
        ...
        "quality": {
            "reportPath": "reports/button_shirt_quality.json",
            "buttonShirtD0Complete": quality["readiness"]["buttonShirtD0Complete"],
        ...
        "warnings": [
            ...
            "button_closures_semantic_not_simulated",
            "phase8_global_partial_after_six_families",
        ],
    }
...
def _quality_report(
    *,
    ...
    appearance: ButtonShirtAppearanceBundle,
    ...
    closure_stations = [int(item["stationIndex"]) for item in pattern["closures"]]
    closure_pairs_valid = (
        len(pattern["closures"]) == 6
        and closure_stations == list(range(6))
        and all(item["paired"] and not item["simulationEnabled"] for item in pattern["closures"])
    )
    fidelity_accepted = appearance.fidelity_report["acceptedForD0ButtonShirtFixture"]
    readiness = all(
        [
            len(pattern["panels"]) == 5,
    ...
    return {
        "schemaVersion": 1,
        "reportVersion": "closy.button_shirt.quality.d0.v1",
        "garmentId": GARMENT_ID,
        "garmentClass": GARMENT_CLASS,
        "topology": {
            ...
            "closureCount": len(pattern["closures"]),
            "openingIds": opening_ids,
            "panelRoles": panel_roles,
            "hasLiteralSplitFront": panel_roles == expected_roles,
            "frontPlacketIsOpen": "opening.button_shirt.front_placket" in opening_ids,
            "closurePairsValid": closure_pairs_valid,
        ...
        "appearance": {
            ...
            "acceptedForD0ButtonShirtFixture": fidelity_accepted,
        },
        "readiness": {
            "buttonShirtD0Complete": readiness,
            "phase8GloballyComplete": False,
            "nextGarmentFamily": "jacket_outerwear",
