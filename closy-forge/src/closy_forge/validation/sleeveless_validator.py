...
from closy_forge.garments.sleeveless_top.fitting import hash_sleeveless_fit_report
from closy_forge.garments.sleeveless_top.motion import hash_sleeveless_motion_report
...
def validate_sleeveless_package(
    ...
    if manifest.get("garmentClass") != "sleeveless_top" or manifest.get("garmentId") != (
        "garment.demo_sleeveless_top.reference_v1"
    ):
        issues.append(
            _issue(
                "sleeveless_family_mismatch",
                "fatal",
                "manifest.json",
                "Sleeveless package identity must use the canonical family and garment IDs.",
            )
        )
    if manifest.get("packageKind") != "closy.garment" or manifest.get("schemaVersion") != 1:
        issues.append(
            _issue(
                "sleeveless_manifest_contract_invalid",
                "fatal",
                "manifest.json",
                "Sleeveless manifest kind/schema is unsupported.",
            )
        )
    if any(
        issue.severity == "fatal" and issue.code == "sleeveless_file_missing" for issue in issues
    ):
        return _report(issues)
    ...
    try:
        ...
        fit = read_json(package_dir / "fitting/sleeveless_fit.json")
        ...
        texture = read_json(package_dir / "textures/sleeveless_pbr_report.json")
        quality = read_json(package_dir / "reports/sleeveless_quality.json")
        binding_manifest = read_json(package_dir / "binding/binding_manifest.json")
    except Exception as exc:
        issues.append(_issue("sleeveless_contract_unreadable", "fatal", ".", str(exc)))
        return _report(issues)
...
def _required_files(package_dir: Path, issues: list[ValidationIssue]) -> None:
    for relpath in sorted(EXPECTED_FILES):
        if not (package_dir / relpath).is_file():
            issues.append(
                _issue(
                    "sleeveless_file_missing",
                    "fatal",
                    relpath,
                    "Required sleeveless package artifact is missing.",
                )
            )
...
def _validate_pattern_semantics(
    ...
    if (
        pattern.get("garmentClass") != "sleeveless_top"
        or semantic.get("garmentClass") != "sleeveless_top"
        ...
        issues.append(
            _issue(
                "sleeveless_semantic_ids_invalid",
                "fatal",
                "semantic/garment_graph.json",
                "Panel, seam and opening IDs must exactly match the sleeveless ontology.",
            )
        )
    all_ids = (
        panel_ids
    ...
    if any(_has_exact_token(identifier, {"sleeve", "cuff"}) for identifier in all_ids):
        issues.append(
            _issue(
                "sleeveless_false_sleeve_semantics",
                "fatal",
                "semantic/garment_graph.json",
                "Sleeveless family must not retain sleeve or cuff semantic tokens.",
            )
        )
    ...
    for seam in pattern.get("seams", []):
        ...
        if len(spans) != 2 or [span.get("orientation") for span in spans] != [
            ...
            issues.append(
                _issue(
                    "sleeveless_seam_orientation_invalid",
                    "fatal",
                    "pattern/pattern.json",
    ...
    if set(resolved_openings) != OPENING_IDS or any(
        ...
        issues.append(
            _issue(
                "sleeveless_opening_loop_invalid",
                "fatal",
                "simulation/constraints.json",
    ...
    if {str(item.get("id")) for item in constraints.get("seams", [])} != SEAM_IDS or any(
        ...
        issues.append(
            _issue(
                "sleeveless_constraint_pairing_invalid",
                "fatal",
                "simulation/constraints.json",
...
def _validate_fit_material_motion(
    ...
    if (
        fit.get("integrity", {}).get("fitReportHash") != hash_sleeveless_fit_report(fit)
        ...
        issues.append(
            _issue(
                "sleeveless_fit_integrity_invalid",
                "fatal",
                "fitting/sleeveless_fit.json",
    ...
    try:
        ...
        issues.append(
            _issue(
                "sleeveless_material_descriptor_invalid",
                "fatal",
                "simulation/material_presets.json",
                str(exc),
            )
    ...
    if (
        ...
        issues.append(
            _issue(
                "sleeveless_material_selection_invalid",
                "fatal",
                "simulation/material_selection.json",
    ...
    if (
        motion.get("integrity", {}).get("suiteHash") != hash_sleeveless_motion_report(motion)
        or {str(item.get("presetId")) for item in records} != PRESET_IDS
        or not all(item.get("actualSolverRun") is True for item in records)
        or not all(item.get("armholeMetrics", {}).get("armholeCount") == 2 for item in records)
        or not all(
        ...
        issues.append(
            _issue(
                "sleeveless_motion_suite_invalid",
                "fatal",
                "reports/material_motion_suite.json",
...
def _validate_geometry_binding(
    ...
    try:
        ...
        issues.append(
            _issue(
                "sleeveless_binding_validation_failed",
                "fatal",
                "binding/sim_to_render.bin",
                str(exc),
            )
...
def _validate_appearance(
    ...
    try:
        ...
        if (
            ...
            or fidelity.get("acceptedForD0SleevelessFixture") is not True
        ...
        issues.append(
            _issue(
                "sleeveless_appearance_validation_failed",
                "fatal",
                "reports/fidelity/source_render_fidelity.json",
                str(exc),
            )
...
def _validate_quality(quality: dict[str, Any], issues: list[ValidationIssue]) -> None:
    ...
    if (
        readiness.get("sleevelessTopD0Complete") is not True
        or readiness.get("phase8GloballyComplete") is not False
        or readiness.get("nextGarmentFamily") != "long_sleeved_top"
        or quality.get("topology", {}).get("hasSleeveOrCuffSemantics") is not False
    ):
        issues.append(
            _issue(
                "sleeveless_quality_claim_invalid",
                "fatal",
                "reports/sleeveless_quality.json",
...
def _validate_inventory(
    ...
    if set(actual_map) != set(declared_map):
        issues.append(
            _issue(
                "sleeveless_inventory_file_set_mismatch",
                "fatal",
                "manifest.json",
    ...
    for relpath, actual_entry in actual_map.items():
        ...
        if (
            ...
            issues.append(
                _issue(
                    "sleeveless_inventory_hash_mismatch",
                    "fatal",
                    relpath,
    ...
    if manifest.get("packageDigest") != canonical_package_digest(actual):
        issues.append(
            _issue(
                "sleeveless_package_digest_mismatch",
                "fatal",
                "manifest.json",
...
def _issue(code: str, severity: Severity, path: str, message: str) -> ValidationIssue:
    return ValidationIssue(
        ...
        path,
        message,
        "Regenerate the sleeveless fixture from canonical Forge inputs.",
    )
