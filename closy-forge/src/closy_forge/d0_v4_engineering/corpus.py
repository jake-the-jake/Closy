...
CORPUS_VERSION = "closy.d0_v4.public_synthetic_tshirt_corpus.v1"
GENERATOR_VERSION = "closy.d0_v4.dual_renderer.v1"
CORPUS_ROOT = Path("fixtures/d0_v4_engineering_corpus_v1")
MANIFEST_PATH = CORPUS_ROOT / "manifest.json"
ARCHIVE_PATH = CORPUS_ROOT / "captures.zip"
PARTITION_COUNTS = {"train": 512, "validation": 128, "public_test": 128}
PARTITION_SEEDS = {
    "train": "closy-d0-v4-public-development-train-2026-03",
    "validation": "closy-d0-v4-public-development-validation-2026-03",
    "public_test": "closy-d0-v4-untouched-public-test-2026-03",
}
...
def generate_corpus(root: Path) -> tuple[dict[str, Any], bytes]:
    ...
    manifest: dict[str, Any] = {
        ...
        "partitionSeeds": {
            role: sha256_bytes(seed.encode("utf-8")) for role, seed in PARTITION_SEEDS.items()
        },
...
def load_manifest(root: Path) -> dict[str, Any]:
    ...
    issues = validate_manifest(value, root=root)
    if issues:
        raise ValueError("d0_v4_corpus_manifest_invalid:" + ";".join(issues))
    return value


def validate_manifest(manifest: Mapping[str, Any], *, root: Path | None = None) -> list[str]:
    issues: list[str] = []
    records = _records(manifest.get("records"))
    if manifest.get("corpusVersion") != CORPUS_VERSION:
        issues.append("corpus_version_invalid")
    observed_counts = {
        role: sum(record.get("partition") == role for record in records)
        for role in PARTITION_COUNTS
    }
...
def load_partition(
    ...
    *,
    ...
    with zipfile.ZipFile(root / ARCHIVE_PATH, "r") as archive:
...
def _generate_partition(
    ...
    permutations = _latin_hypercube_permutations(count, len(OBSERVABLE_PARAMETERS), rng)
    records: list[dict[str, Any]] = []
    for ordinal in range(count):
        identity_seed = hashlib.sha256(
            f"{PARTITION_SEEDS[partition]}:{ordinal}".encode()
        ).hexdigest()
        local = random.Random(identity_seed)
        parameters = {
            name: round(
                low + (high - low) * ((permutations[index][ordinal] + local.random()) / count),
                9,
            )
...
def render_tshirt_capture(
    ...
    *,
    ...
    shoulder = (25.0 + 8.0 * normalized["shoulder_width"]) * scale
    ...
    sleeve = (12.0 + 22.0 * normalized["sleeve_length"]) * scale
    cuff = (5.0 + 9.0 * normalized["sleeve_opening_width"]) * scale
    hem_half = chest + ease
    left = [
        (center - shoulder, top + slope),
        (center - shoulder - sleeve, top + slope + cuff),
        (center - shoulder - sleeve + 2.0, top + slope + cuff * 2.0),
        (center - chest, top + armhole),
    ...
    _ellipse(
        draw,
        (center - neck_width, top - 2.0, center + neck_width, top + neck_depth),
