...
BUTTON_SHIRT_FIT_VERSION = "closy.button_shirt.bounded_fit.d0.v1"


def fit_button_shirt(
    prior: ButtonShirtParameters,
    *,
    observed_half_width_meters: float = 0.347,
    observed_body_length_meters: float = 0.682,
    observed_sleeve_length_meters: float = 0.578,
    observed_placket_length_meters: float = 0.595,
) -> tuple[ButtonShirtParameters, dict[str, Any]]:
    prior.validate()
    observations = {
        ...
        "placketLengthMeters": observed_placket_length_meters,
        "source": "public_synthetic_button_shirt_capture_fixture",
    ...
    for index, (width_delta, length_delta) in enumerate(
        ...
        losses["weightedObjective"] = _round(
            ...
            + losses["placketLengthErrorMeters"] * 0.22
        )
        evaluations.append(
            {
                "candidateId": f"candidate.button_shirt.{index:03d}",
    ...
    winner = ButtonShirtParameters(**winner_doc["parameters"])
    accepted = float(winner_doc["losses"]["weightedObjective"]) <= 0.005
    report: dict[str, Any] = {
        "schemaVersion": 1,
        "fitVersion": BUTTON_SHIRT_FIT_VERSION,
        "fitReportId": "fit.button_shirt.public_d0_v1",
        "garmentClass": "button_shirt",
    ...
    report["integrity"]["fitReportHash"] = hash_button_shirt_fit_report(report)
    return winner, report


def hash_button_shirt_fit_report(report: dict[str, Any]) -> str:
