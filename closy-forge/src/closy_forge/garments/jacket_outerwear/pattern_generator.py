...
from .parameters import JacketOuterwearParameters

GARMENT_ID = "garment.demo_jacket_outerwear.reference_v1"
GARMENT_CLASS = "jacket_outerwear"
PREFIX = "jacket_outerwear"
MATERIAL_REGION = "material.heavy_jersey_reference_v1"


def build_jacket_outerwear_pattern(params: JacketOuterwearParameters) -> dict[str, Any]:
    params.validate()
    ...
    panels = [
        ...
        _facing_panel("left", params, target),
        _facing_panel("right", params, target),
    ]
    return {
        "schemaVersion": 1,
        "patternVersion": "closy.jacket_outerwear.pattern.d0.v1",
        ...
        "provenance": {
            ...
            "generator": "closy.jacket_outerwear.pattern.d0.v1",
...
def _front_panel(side: str, params: JacketOuterwearParameters, target: float) -> dict[str, Any]:
    ...
    if side == "left":
        boundary = [
            ...
            _line(
                f"edge.{PREFIX}.facing_attachment.left",
                (0.0, 0.0),
                (0.0, neck_center_y),
                target,
            ),
        ...
        boundary = [
            ...
            _line(
                f"edge.{PREFIX}.facing_attachment.right",
                (0.0, neck_center_y),
                (0.0, 0.0),
                target,
            ),
...
def _back_panel(params: JacketOuterwearParameters, target: float) -> dict[str, Any]:
    half = params.half_chest_width_meters + params.body_ease_meters
    shoulder_half = params.shoulder_width_meters / 2
    neck_half = params.neckline_width_meters / 2
    length = params.body_length_meters
...
def _sleeve_panel(side: str, params: JacketOuterwearParameters, target: float) -> dict[str, Any]:
...
def _facing_panel(side: str, params: JacketOuterwearParameters, target: float) -> dict[str, Any]:
    width = params.facing_width_meters
    length = params.body_length_meters
    neck_y = length - params.front_neckline_depth_meters
    sign = -1.0 if side == "left" else 1.0
    outer_x = 0.0
    inner_x = sign * width
    panel_id = f"panel.{PREFIX}.facing.{side}"
    if side == "left":
        boundary = [
            _line(f"edge.{PREFIX}.facing.hem.left", (inner_x, 0.0), (outer_x, 0.0), target),
            _line(
                f"edge.{PREFIX}.facing.outer.left",
                (outer_x, 0.0),
                (outer_x, neck_y),
                target,
            ),
            _line(
                f"edge.{PREFIX}.facing.neck.left",
                (outer_x, neck_y),
                (inner_x, length),
                target,
            ),
            _line(
                f"edge.{PREFIX}.facing.inner.left",
                (inner_x, length),
                (inner_x, 0.0),
                target,
            ),
        ...
        boundary = [
            _line(f"edge.{PREFIX}.facing.hem.right", (outer_x, 0.0), (inner_x, 0.0), target),
            _line(
            ...
            _line(
                f"edge.{PREFIX}.facing.neck.right",
                (inner_x, length),
                (outer_x, neck_y),
                target,
            ),
            _line(
                f"edge.{PREFIX}.facing.outer.right",
                (outer_x, neck_y),
                (outer_x, 0.0),
                target,
            ),
    ...
    return {
        "id": panel_id,
        "partId": f"part.{PREFIX}.front_facing_{side}",
        "semanticRole": f"front_{side}_facing",
        "coordinateSystem": "panel-local-metres",
        "grainDirection": [0.0, 1.0],
        "materialRegion": MATERIAL_REGION,
        "symmetry": f"panel.{PREFIX}.facing.{'right' if side == 'left' else 'left'}",
        "seamAllowance": params.seam_allowance_meters,
        "tessellation": {"targetEdgeLength": target},
        "confidence": {"source": "authored_deterministic_fixture", "value": 1.0},
        "boundary": boundary,
...
def _seams(params: JacketOuterwearParameters, target: float) -> list[dict[str, Any]]:
    ...
    for offset, side in enumerate(("left", "right")):
        ...
        seams.extend(
            [
                ...
                _seam(
                    f"seam.{PREFIX}.facing_attachment.{side}",
                    _span(front, f"edge.{PREFIX}.facing_attachment.{side}"),
                    _span(
                        f"panel.{PREFIX}.facing.{side}",
                        f"edge.{PREFIX}.facing.outer.{side}",
                    ),
                    50 + offset,
                ),
            ]
    ...
    return [
        ...
        _opening(
            ...
            [
                ...
                f"edge.{PREFIX}.facing.hem.left",
                f"edge.{PREFIX}.facing.hem.right",
            ],
        ),
        _opening(
            f"opening.{PREFIX}.front",
            [f"edge.{PREFIX}.facing.inner.left", f"edge.{PREFIX}.facing.inner.right"],
        ),
