...
JACKET_OUTERWEAR_FIT_VERSION = "closy.jacket_outerwear.bounded_fit.d0.v1"


def fit_jacket_outerwear(
    prior: JacketOuterwearParameters,
    *,
    observed_half_width_meters: float = 0.377,
    observed_body_length_meters: float = 0.722,
    observed_sleeve_length_meters: float = 0.598,
    observed_front_opening_length_meters: float = 0.625,
) -> tuple[JacketOuterwearParameters, dict[str, Any]]:
    prior.validate()
    observations = {
        ...
        "frontOpeningLengthMeters": observed_front_opening_length_meters,
        "source": "public_synthetic_jacket_outerwear_capture_fixture",
    ...
    for index, (width_delta, length_delta) in enumerate(
        ...
        losses = {
            ...
            "frontOpeningLengthErrorMeters": _round(
                abs(
                    candidate.body_length_meters
                    - candidate.front_neckline_depth_meters
                    - observed_front_opening_length_meters
                )
            ),
        }
        losses["weightedObjective"] = _round(
            losses["halfWidthErrorMeters"] * 0.34
            + losses["bodyLengthErrorMeters"] * 0.24
            + losses["sleeveLengthErrorMeters"] * 0.20
            + losses["frontOpeningLengthErrorMeters"] * 0.22
        )
        evaluations.append(
            {
                "candidateId": f"candidate.jacket_outerwear.{index:03d}",
    ...
    winner = JacketOuterwearParameters(**winner_doc["parameters"])
    accepted = float(winner_doc["losses"]["weightedObjective"]) <= 0.005
    report: dict[str, Any] = {
        "schemaVersion": 1,
        "fitVersion": JACKET_OUTERWEAR_FIT_VERSION,
        "fitReportId": "fit.jacket_outerwear.public_d0_v1",
        "garmentClass": "jacket_outerwear",
    ...
    report["integrity"]["fitReportHash"] = hash_jacket_outerwear_fit_report(report)
    return winner, report


def hash_jacket_outerwear_fit_report(report: dict[str, Any]) -> str:
