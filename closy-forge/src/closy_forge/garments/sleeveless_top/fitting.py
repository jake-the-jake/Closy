...
SLEEVELESS_FIT_VERSION = "closy.sleeveless_top.bounded_fit.d0.v1"


def fit_sleeveless_top(
    prior: SleevelessTopParameters,
    *,
    observed_half_width_meters: float = 0.327,
    observed_body_length_meters: float = 0.642,
    observed_armhole_depth_meters: float = 0.226,
) -> tuple[SleevelessTopParameters, dict[str, Any]]:
    ...
    observations = {
        ...
        "source": "public_synthetic_sleeveless_capture_fixture",
        "measuredFromPrivateImage": False,
    }
    evaluations: list[dict[str, Any]] = []
    candidates: list[SleevelessTopParameters] = []
    ...
    for index, candidate in enumerate(candidates):
        ...
        evaluations.append(
            {
                "candidateId": f"candidate.sleeveless_top.{index:03d}",
    ...
    winner = SleevelessTopParameters(**winner_doc["parameters"])
    accepted = float(winner_doc["losses"]["weightedObjective"]) <= 0.005
    report: dict[str, Any] = {
        "schemaVersion": 1,
        "fitVersion": SLEEVELESS_FIT_VERSION,
        "fitReportId": "fit.sleeveless_top.public_d0_v1",
    ...
    report["integrity"]["fitReportHash"] = hash_sleeveless_fit_report(report)
    return winner, report


def hash_sleeveless_fit_report(report: dict[str, Any]) -> str:
