...
class SleevelessAppearanceBundle:
    ...
    *, pattern: dict[str, Any], settled_mesh: MeshSet, seed: int
    ...
    for label, panel_id in (
        ...
        rendered = rasterize_settled_garment(
            ...
            width=WIDTH,
            height=HEIGHT,
            texture_sampler=lambda _panel_id, _uv: FABRIC_RGBA,
        )
        ...
        source_views.append(
            {
                "viewId": f"view.sleeveless_top.{label}",
                "label": label,
                "sourcePath": source_path,
                "sourceSha256": sha256_bytes(source_bytes),
                "decodedPixelHash": _pixel_hash(source),
    ...
    texture_artifacts, texture_report = _texture_artifacts(source_views)
    artifacts.update(texture_artifacts)
    capture_record = {
        "schemaVersion": 1,
        "recordVersion": "closy.sleeveless_top.capture_fixture.d0.v1",
        "recordId": "capture.synthetic_sleeveless_top_reference_v1",
    ...
    fidelity_report = {
        "schemaVersion": 1,
        "reportVersion": "closy.sleeveless_top.source_render_fidelity.d0.v1",
        "reportId": "source_render_fidelity.sleeveless_top.public_d0_v1",
        "garmentId": pattern["garmentId"],
        "garmentClass": "sleeveless_top",
        "status": (
            "pass_d0_public_fixture"
            if all(view["accepted"] for view in fidelity_views)
            else "partial_d0_public_fixture"
        ),
        ...
        "aggregate": aggregate,
        "acceptedForD0SleevelessFixture": all(view["accepted"] for view in fidelity_views),
        "acceptedForProductionVisualFidelity": False,
    }
    return SleevelessAppearanceBundle(
        artifacts=artifacts,
...
def _source_fixture(panel: dict[str, Any]) -> DecodedPng:
...
def _texture_artifacts(
    source_views: list[dict[str, Any]],
) -> tuple[dict[str, bytes | dict[str, Any]], dict[str, Any]]:
    pixels: dict[str, bytes] = {
        "base_color": bytes(FABRIC_RGBA) * (ATLAS_SIZE * ATLAS_SIZE),
    ...
    maps = [
        {
            "mapId": key,
            "path": paths[key],
            "sha256": sha256_bytes(_bytes(artifacts[paths[key]])),
            "width": ATLAS_SIZE,
            "height": ATLAS_SIZE,
    ...
    report = {
        "schemaVersion": 1,
        "appearanceVersion": SLEEVELESS_APPEARANCE_VERSION,
        "textureIdentityId": "texture.sleeveless_top.public_d0_v1",
    ...
    artifacts["textures/sleeveless_pbr_report.json"] = report
